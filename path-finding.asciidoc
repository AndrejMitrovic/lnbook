...
Let us use our standard example in which Alice wants to send a payment of 100k satoshi on a path via Bob and Charlie to Gloria.
The _path_ obviously looks like (Alice)-->(Bob)-->(Charlie)-->Gloria.
Bob and Charlie will charge routing fees to forward the _onion_.
...
For simplicity assume that the fee rate of Bob and Charlie is expensive with 1% for Bob and 2% for Charlie.
For simplicity furthermore assume neither Bob nor Charlie take a base fee.
...
Similarly she knows that 2% of 100k satoshi is 2k satoshi which she belives she should include in Charlie's onion.
...
In the next step she would have added Charlie's 2% fee to compute 102k for Charlie's input.
...
[[rebalancing-2]]
.The Bob - Charlie channel becomes now imbalanced
image:images/rebalancing-2.png[]

you can see that after Bob made a payment of 1 million satoshi to Charlie the channel balance was shifted.
Bob now has 1.5 million satoshi on the channel and Charlie has 3.5 million satoshi on the channel.
The balance ratio went from 50/50 to 30/70.
The other 2 channels however styed with 50/50.

Charlie decides that he wants to have a 50/50 channel with Bob.
...
Thus there is only the last option which means that Charlie could move the money from the Bob-Charlie channel via the Bob-Erica channel to hhis Erica-Charlie channel.

[[rebalancing-4]]
.Charlie tries to rebalance the Bob-Charlie channel in the unbalanced network via a circular onion of 1 mio Satoshi.
image:images/rebalancing-4.png[]

The problem in the new network can easily be seen on the next picture.
While the Bob-Charlie channel now becomes 50/50 again all the other channel turned into a 30/70 split ratio.
...
After the payment and the rebalancing it looked like Bob initially had sent Money not via the Bob-Charlie channel but via the path along Erica.
...
Remember in our example after Bob has paid Charlie Bob had a total amount of 4 million satoshi, Charlie had a total of 6 million satoshi and Erica still had 5 million satoshi as before.
...
Let us elaborate on the second point and take the example in which Bob was supposed to forward the onion from Alice to Charlie but does have enough liquidity on the channel with Charlie.
...
One part flows directly to Charlie and the other part takes the path over Erica to Charlie.
