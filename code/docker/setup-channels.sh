...
wei_address=$(docker-compose exec -T Charlie bash -c "eclair-cli -s -j -p eclair getinfo| jq .nodeId")
...
echo Bob to Charlie
docker-compose exec -T Bob lightning-cli connect ${wei_address}@Charlie
docker-compose exec -T Bob lightning-cli fundchannel ${wei_address} 1000000

echo Charlie to Gloria
docker-compose exec -T Charlie eclair-cli -p eclair connect --uri=${gloria_address}@Gloria
docker-compose exec -T Charlie eclair-cli -p eclair open --nodeId=${gloria_address} --fundingSatoshis=1000000
