...
Bob, in turn, has an open channel with the software developer Charlie who helps him with the point of sale system he uses in his coffee shop.
Charlie is also the owner of a large software company which develops the game that Gloria plays, and they already have an open channel which Gloria uses to pay for the game's license and in-game items.

If we draw out this series of payment channels, it's possible to manually trace a _path_ from Alice to Gloria that uses Bob and Charlie as intermediary routing nodes.
Alice can then craft a _route_ from this outlined path, and use it to send a tip of a few thousand satoshis to Gloria, with the payment being _forwarded_ by Bob and Charlie.
Essentially, Alice will pay Bob, who will pay Charlie, who will pay Gloria.
...
The main challenge is to do this in a way that prevents Bob and Charlie from stealing the money that Alice wants delivered to Gloria.
...
However, Alice knows Bob, who knows Charlie, who knows Gloria and so she decides to ask Bob and Charlie for help.
She can pay Bob to pay Charlie to pay Gloria, but how does she make sure that Bob or Charlie don't run off with the coins after receiving them?
...
====
_I (Alice) will give you (Bob) 10 gold coins if you pass them on to Charlie_
====
...
====
_I (Alice) will reimburse you (Bob) with 10 gold coins if you can prove to me (for example via a receipt) that you already have delivered 10 gold coins to Charlie_
====

You might ask yourself why should Bob sign such a contract.
He has to pay Charlie but ultimately gets nothing out of the exchange, and he runs the risk that Alice might not reimburse him.
Bob could offer Charlie a similar contract to pay Gloria, but similarly Charlie has no reason to accept it either.
Even putting aside the risk, Bob and Charlie must _already_ have 10 gold coins to send, otherwise they wouldn't be able to participate in the contract.
Thus Bob and Charlie face both risk and opportunity cost for agreeing to this contract, and they would need to be compensated in order for them to accept it.

Alice can this make this attractive to both Bob and Charlie, by offering them fees of 1 gold coin each, if they transmit her payment to Gloria.
The final contract would instead read:

[alice-gloria-routing-3]
====
_I (Alice) will reimburse you (Bob) with 12 gold coins if you can prove to me (for example via a receipt) that you already have delivered 11 golden coins to Charlie_
====

Alice now promises Bob 12 gold coins.
There are 10 to be delivered to Gloria and 2 for the fees.
She promises 12 to Bob if he can prove that he has forwarded 11 to Charlie.
...
At the start of the exchange, Alice could "lock up" these 12 golden coins in escrow that will only be paid to Bob once he proves that he's paid 11 golden coins to Charlie.
...
You can acquire this message by setting up a similar Contract with Charlie who has to set up a similar contract with Gloria.
...
This new contract now protects Alice from Bob not forwarding to Charlie, protects Bob from not being reimbursed by Alice, and ensures that there will be proof that Gloria was ultimately paid via the hash of Gloria's secret.
This secret message that hashes to the +f23c83...+ is called a _pre-image_.

After Bob and Alice agree to the contract, and Bob receives the message from the escrow that Alice has deposited the 12 gold coins, Bob can now negotiate a similar contract with Charlie.

Note that since Bob is taking a service fee of 1 coin, he will only forward 11 gold coins to Charlie once Charlie shows proof that he has paid Gloria.
Similarly, Charlie will also demand a fee and will expect to receive 11 gold coins once he has proved that he has paid Gloria the promised 10 gold coins.

Bob's contract with Charlie will read:

[alice-gloria-routing-5]
====
_I (Bob) will reimburse you (Charlie) with 11 gold coins if you can show me a valid message that hashes to:`+f23c83...+`.
...
Once Charlie gets the message from the escrow that Bob has deposited the 11 gold coins, Charlie sets up a similar contract with Gloria:

[alice-gloria-routing-6]
====
_I (Charlie) will reimburse you (Gloria) with 10 golden coins if you can show me a valid message that hashes to:`+f23c83...+`.
...
Bob has a contract with Charlie and has placed 11 gold coins in escrow
Charlie has a contract with Gloria and has placed 10 gold coins in escrow.
It is now up to Gloria to reveal the secret, which is the pre-image to the hash she has established as proof of payment.

Gloria now sends +"Glorias secret"+ to Charlie.

He checks that +"Glorias secret" hashes to +f23c83...+. Charlie now has proof of payment and so instructs the escrow service to release the 10 golden coins to Gloria.

Charlie now provides the secret to Bob. Bob checks it and instructs the escrow service to release the 11 gold coins to Charlie.
...
Alice has paid a total of 12 gold coins, 1 of which was received by Bob, 1 of which was received by Charlie, and 10 of which were received by Gloria.
With a chain of contracts like this in place, Bob and Charlie could not run away with the money because they deposited it in escrow first.

However, one issue still remains.
If Gloria refused to release her secret pre-image, then Charlie, Bob, and Alice would all have their coins stuck in escrow but wouldn't be reimbursed.
...
Even if he successfully pays Charlie, if he receives the proof of payment later than 24 hours he will not be reimbursed. To remove that risk, Bob must give Charlie and even shorter deadline.

In turn, Bob will alter his contract with Charlie in the following way:

[alice-gloria-routing-8]
====
_Charlie has 22 hours to show the secret after the contract was signed.
...
As you might have guessed, Charlie is now incentivized to also alter his contract with Gloria:
...
With such a chain of contracts we can ensure that, after 24 hours, the payment will successfully deliver from Alice to Bob to Charlie to Gloria, or it will fail and everyone will be refunded.
...
Lets stick to our example in which Alice still wants to tip Gloria and has decided to use the path via Bob and Charlie.
...
For example after Bob received the onion from Alice he will be able to decrypt the first layer and he will only see the information that he is supposed to forward the onion to Charlie by setting up an HTLC with Charlie.
The HTLC with Charlie should use the same Payment Hash as the receiving HTLC from Alice.
...
He removes the information that he could read from it and passes it along to Charlie.

Charlie in turn is only able to see that he is supposed to forward the package to Gloria.
Charlie knows he received the onion from Bob but has no clue that it was actually Alice who initiated the onion in the first place.
...
For example Bob will explicitly be told that Alice offered him an HTLC and sent him an onion and that he is supposed to offer an HTLC to Charlie and forward a slightly modified onion.
...
Due to the layered encryption he cannot see the inside of Charlie's, and Gloria's layer.
The only thing Bob is told explicitly is that he was involved in a path that involved Alice, him and Charlie.

While the Onion is decrypted layer by layer while it travels along the path from Alice via Bob and Charlie to Gloria it is created from the inside layer to the outside layers via several rounds of encryption.
...
After the encrypted Onion for David is created Alice will create the next outer layer by creating the onion for Charlie.

She truncates 65 Bytes from the end of the encrypted onion and prepends the truncated onion with 65 Byte per Hop data for Charlie.
...
This is set to 452 as Charlie is meant to use the channel with this channel ID as the next outgoing channel.
She sets the amount to 3000 satoshi as this is the amount that David is supposed to receive.
Finally she uses the CLTV delta added to the current height that was announced for this channel on the gossip protocol and that Charlie should use for the HTLC when he forwards the Onion.
...
We emphasize that Charlie cannot decrypt the inner part of the onion (that's still encrypted from his PoV), as he cannot derive the encryption keys.
However the information for Charlie should also be protected from others.
Thus Alice conducts another ECDH.
This time with Charlie's public key and the randomized ephemeral key pair.
She uses the shared secret to encrypt the onion payload.
She would be able to construct the entire onion for Charlie - which actually Bob does while he forwards the onion.
The Onion that Charlie would receive can be seen in the following diagram:

[[routing-onion-5]]
.`per_hop` payload of Glorias onion and the encrypted
image:images/routing-onion-5.png[]

Note that in the entire onion there will be Charlie's ephemeral public key.
...
Luckily the ephemeral keys that Alice used for the ECDH with David can be derived from the ephemeral key that she used for Charlie.
Thus after Charlie decrypts his layer he can use the shared secret and his ephemeral public key to derive the ephemeral public key that David is supposed to use and store it in the header of the Onion that he forwards to David.
The exact progress to generate the ephemeral keys for every hope will be explained at the very end of the chapter.
Similarly it is important to recognize that Alice removed data from the end of Davids onion payload to create space for the per hop data in Charlie's onion.
Thus when Charlie has received his onion and removed his routing hints and per hop data the onion would be to short and he somehow needs to be able to append the 65 Bytes of filled junk data in a way that the HMACs will still be valid.
...
Finally after Alice has computed the encrypted version for Charlie she will use the exact same process to compute the encrypted version for Bob.
For Bobs onion she actually computes the header and provides the ephemeral public key herself.
Note how Charlie was still supposed to forward 3000 satoshis but How Bob was supposed to forward a different amount.
The difference is the routing fee for Charlie.
...
Recall for example the situation in which Alice sent and onion to Bob who set up an HTLC with Charlie.
If Alice wanted to remove the HTLC with Bob this would put a financial risk on Bob.
He fears that his HTLC with Charlie still might be fulfilled meaning that he could not claim the reimbursement from Alice.
Thus Bob would never agree to remove the HTLC with Alice unless he already has removed his HTLC with Charlie.
If however the HTLC between Alice and Bob are set up and the HTLC between Bob and Charlie are set up but Charlie encounters problems with forwarding the onion it is perfectly Charlie has more options than Alice.
While sending back the error Onion to Bob Charlie could ask him to remove the HTLC.
Bob has no risk in removing the HTLC with Charlie and Charlie also has no risk as there is no downstream HTLC.
